...
interface EmaSwingChecklistPageProps {
...
export const EmaSwingChecklistPage: React.FC<EmaSwingChecklistPageProps> = ({
  checklist,
  onToggleItem,
  onResetChecklist,
  onNavigate,
  ...
  const isAllConfirmed = totalCount === 4 && completedCount === 4;
    ...
    <div className="mx-auto max-w-3xl space-y-6 px-3 py-2 sm:px-4 sm:py-4">
      ...
      <div className="flex flex-col gap-3 rounded-xl border border-[#2A2D35] bg-[#121620] p-4 sm:flex-row sm:items-center sm:justify-between shadow-lg">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <div className="flex items-center space-x-2">
              ...
              <span className="rounded bg-violet-500/10 border border-violet-500/30 px-2 py-0.5 font-mono text-[10px] font-bold text-violet-300">
                H1 / H4
              </span>
            </div>
            <p className="text-xs text-gray-400 font-medium">H1/H4 Pre-Trade Checklist</p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between sm:justify-end space-x-3">
          ...
          <div className="flex items-center space-x-2 rounded-lg border border-[#2A2D35] bg-[#0B0E14] px-3 py-1.5 font-mono text-xs">
            ...
            <span
              className={`font-bold ${
                isAllConfirmed ? 'text-emerald-400' : 'text-violet-400'
              }`}
            >
      ...
      <div className="space-y-3.5">
        {checklist.map((item, index) => {
          ...
          const timeframe = item.timeframe || 'H1 / H4';
          const instruction = item.instruction || item.description || item.label;

          return (
            <div
              key={item.id}
              id={`ema-step-card-${stepNum}`}
              className={`rounded-xl border p-4 sm:p-5 transition-all select-none ${
                item.checked
                  ? 'border-violet-500/40 bg-gradient-to-r from-[#1E152E] to-[#121622] shadow-[0_0_15px_rgba(139,92,246,0.08)]'
                ...
                <button
                  ...
                  className={`flex items-center space-x-2.5 rounded-lg border px-3.5 py-2 text-xs font-semibold tracking-wide transition-all ${
                    item.checked
                      ...
                      : 'border-[#2A2D35] bg-[#1A1F2C] text-gray-300 hover:border-violet-500/60 hover:text-white'
                  }`}
                >
      ...
      >
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start space-x-3.5">
            ...
            <div className="space-y-1">
              <div className="flex items-center space-x-2">
                ...
                >
                  {isAllConfirmed ? '4 / 4 READY' : `${completedCount} / 4 COMPLETE`}
                </span>
              </div>
              ...
              {!isAllConfirmed ? (
                ...
                <p className="text-xs text-emerald-200/90 font-medium">
                  All 4 EMA Swing criteria verified. Proceed to Risk Calculator & Trade Execution.
                </p>
              )}
            </div>
          </div>
          ...
          <div className="flex sm:flex-col items-center sm:items-end gap-2 shrink-0 pt-2 sm:pt-0">
            <button
              id="btn-ema-proceed-risk"
              onClick={() => {
                soundEngine.play('button_click');
                onNavigate('RISK_CALCULATOR');
              }}
              className="flex w-full items-center justify-center space-x-2 rounded-lg border border-violet-500/40 bg-violet-600/30 px-4 py-2 text-xs font-bold text-violet-200 transition-all hover:bg-violet-600/50 hover:text-white"
            >
